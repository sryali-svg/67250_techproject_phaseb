...
function changeSlide(slideshowName, direction) {
    const slideshow = document.querySelector(`.${slideshowName}-slideshow`);
    const slides = slideshow.querySelectorAll(".slide");

    slides[slideIndexes[slideshowName]].classList.remove("active");

    slideIndexes[slideshowName] += direction;

    if (slideIndexes[slideshowName] < 0) {
        slideIndexes[slideshowName] = slides.length - 1;
    }

    if (slideIndexes[slideshowName] >= slides.length) {
        slideIndexes[slideshowName] = 0;
    }

    slides[slideIndexes[slideshowName]].classList.add("active");
}